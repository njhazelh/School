/* Name: Nicholas Jones
 * Email: njhazelh@zimbra.husky.neu.edu
 * Comments: n/a
 */
...
/**
 * BTreeTimer is a class to control timing trials for BTree.
 * After construction, the user sets the values of BTreeTimer.
 * Once all important values (see runTrial) have been set, the
 * user calls runTrial to run timings.  trialToString then
 * gets the result.
 * @author Nick Jones
...
public class BTreeTimer {
    ...
    /**
     * Set the number of words to read into the file. As BTree is implemented
     * a negative value or a value greater than the number of words in this.file
    ...
    /** How long does it take to run contains on all words in the file
     * referenced by containsFile
     * Build must be run before this is called.
    ...
    private long timeContains() {
        long start = System.currentTimeMillis();
        for (String s : new StringIterator(containsFile)) {
    ...
    /**
     * How long does it take to iterate through all words in the BTree?
     * build must be run before this is called.
     * @return An ArrayList where get(0) is the number of milliseconds it
     * took to iterate, and get(1) is the number of words iterated though.
     */

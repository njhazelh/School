...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(list 'sqrt x) (Sqrt (parse-sexpr x))]
    [else (error 'parse-sexpr "bad syntax in ~s" sexpr)]))
;; Test
(test (parse-sexpr (list 'x 3 5)) =error> "bad syntax in")
...
(define (sqrt+ ns)
  (cond [(null? ns) null]
        ...
        [else (let ([pos-root (sqrt (first ns))])
                (cons pos-root
                      (cons (- pos-root)
                            (sqrt+ (rest ns)))))]))
...
(test (run "{with {x 5} {/ 25 x}}") => '(5))
(test (run "{with {x 25} {sqrt x}}") => '(5 -5))
(test (run "{with {x {sqrt 100}} {+ x 10}}") => '(20 0))
(test (run "{sqrt 1}") => '(1 -1))
(test (run "{sqrt 9}") => '(3 -3))
(test (run "{sqrt 0}") => '(0 0))

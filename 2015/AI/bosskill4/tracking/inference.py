...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        posDist = self.getBeliefDistribution()
        newParticles = []
        ...
        if noisyDistance is None:
            ...
            for p in posDist:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                posDist[p] = posDist[p] * emissionModel[trueDistance]
            posDist.normalize()
            if posDist.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for i in xrange(self.numParticles):
                    newParticles.append(util.sample(posDist))
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            posDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            newParticles.append(util.sample(posDist))
        self.particles = newParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        counts = util.Counter()
        particles = []
        perms = itertools.product(self.legalPositions, repeat=self.numGhosts)
        queue = util.PriorityQueue()
        for p in perms:
            queue.push(p, 0)
        for i in xrange(self.numParticles):
        ...
        for i in xrange(self.numGhosts):
            if noisyDistances[i] is not None:
                for p in posDist:
                    ...
                    posDist[p] = posDist[p] * emissionModels[i][trueDistance]
        posDist.normalize()
        if posDist.totalCount() == 0:
            self.initializeParticles()
        else:

...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ps4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
#|
Nicholas Jones
Zachary Wassall
Problem Set 4
...
(define EDITOR_PAD_X 4) ;; The (4,4) placement; although, the directions
(define EDITOR_PAD_Y 4) ;; are kinda very vague.
(define TEXT_COLOR "black")
(define FONT_SIZE 20)
(define CURSOR_WIDTH 1)
(define CURSOR_HEIGHT 25)
(define CURSOR_COLOR "red")
(define CURSOR_IMAGE (rectangle CURSOR_WIDTH
                                CURSOR_HEIGHT
                                "solid"
                                CURSOR_COLOR))
(define BACKGROUND (empty-scene   EDITOR_WIDTH
                                  EDITOR_HEIGHT ))
...
(check-expect (cursor-left (make-editor "" "test")) (make-editor "" "test"))
...
(check-expect (cursor-right (make-editor "test" "")) (make-editor "test" ""))
...
(check-expect (type-unsafe (make-editor "00000000000000000000000" "1111") "2")
              (make-editor "000000000000000000000002" "1111"))
...
(define (type editor key)
  (cond [(> (+ (image-width (render (type-unsafe editor
                                                 key)))
               (* 2 EDITOR_PAD_X))
            EDITOR_WIDTH)
         editor]
...
(check-expect (type (make-editor "000000000000000000000000000000" "1111") "2")
              (make-editor "000000000000000000000000000000" "1111"))
...
(check-expect (edit e1 "a")       (make-editor "hello a" "world"))
(check-expect (edit e1 "control") e1)
...
(define (editor->scene editor)
  (place-image (render editor)
               (+ EDITOR_PAD_X
                  (quotient (image-width (render editor))
                     2))
               (+ EDITOR_PAD_Y
                  (quotient (image-height (render editor))
                     2))
               BACKGROUND))
...
;; Examples:
;;    "hello" -> (text "hello" FONT_SIZE TEXT_COLOR)
;;    "world" -> (text "world" FONT_SIZE TEXT_COLOR)
...
;; TESTS
(check-expect (string-render "hello") (text "hello"
                                            FONT_SIZE
                                            TEXT_COLOR))
(check-expect (string-render "world") (text "world"
                                            FONT_SIZE
                                            TEXT_COLOR))
...
(define (render editor)
  (place-image CURSOR_IMAGE
               (image-width (string-render (editor-pre editor)))
               (quotient EDITOR_HEIGHT 2)
...
;; An Editor is a (make-editor String Number) where
...
(check-expect (cursor-left (make-editor "test" 0)) (make-editor "test" 0))
...
(define (cursor-right editor)
  (cond [(= (string-length (editor-text editor))
            (editor-cursor editor)) 
         editor]
        [else (make-editor (editor-text editor)
                           (+ (editor-cursor editor) 1))]))
...
;; TESTS
(check-expect (cursor-right e1) (make-editor "hello world" 7))
(check-expect (cursor-right e2) (make-editor "0123456789" 6))
(check-expect (cursor-right (make-editor "test" 4)) (make-editor "test" 4))



;; type-unsafe : Editor Key -> Editor
;; Purpose     : Inserts the key into the editor text at the current
;;               cursor index regardless of size constraints.
;; Examples:
...
;;   (make-editor "0000000000000000000000000000000000000000021111" 42)
(define (type-unsafe editor key)
  (make-editor (string-append (substring (editor-text editor)
                                         0
                                         (editor-cursor editor))
                              key
...
;; TESTS
(check-expect (type-unsafe e1 "1") (make-editor "hello 1world" 7))
(check-expect (type-unsafe e2 "h") (make-editor "01234h56789" 6))
(check-expect (type-unsafe (make-editor "000000000000000000000001111" 23) "2")
              (make-editor "0000000000000000000000021111" 24))
...
(check-expect (type (make-editor "0000000000000000000000000000001111" 34) "2")
              (make-editor "0000000000000000000000000000001111" 34))



;; delete  : Editor -> Editor
;; Purpose : Deletes the character immediately before the cursor
;; Examples:
;;    e1 -> (make-editor "helloworld" 5)
;;    e2 -> (make-editor "012356789" 4)
;;    (make-editor "1234" 0) -> (make-editor "1234" 0)
;;    (make-editor "" 0) -> (make-editor "" 0)
(define (delete editor)
  (cond [(= 0
            (editor-cursor editor))
         editor]
        [else (make-editor (string-append (substring (editor-text editor)
                                                     0
                                                     (- (editor-cursor editor)
                                                        1))
                                          (substring (editor-text editor)
...
;; TESTS
(check-expect (delete e1) (make-editor "helloworld" 5))
(check-expect (delete e2) (make-editor "012356789" 4))
(check-expect (delete (make-editor "1234" 0)) (make-editor "1234" 0))
(check-expect (delete (make-editor "" 0)) (make-editor "" 0))
...
(define (render editor)
  (place-image CURSOR_IMAGE
               (image-width (string-render (substring (editor-text editor)
                                                      0
                                                      (editor-cursor editor))))
               (/ EDITOR_HEIGHT 2)
               (string-render (editor-text editor))))

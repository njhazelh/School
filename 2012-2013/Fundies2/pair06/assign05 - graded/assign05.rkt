...
;;> Final Grade: 168/170
;;> Maximum points for this assignment: <+170>
...
;;> Probably would've made your life easier if status was just a boolean
(define-class wrapper%
  ...
  (define (on-mouse x y me)
    (new wrapper%
         ...
         (map (λ (wld) (if (wld . mouse-over? x y)
              ...
              (this . worlds))
         (this . status)))
  ;;> You can only use the methods outlined in the "WORLD" interface
  ;;> Here you are assuming that a world has a "mouse-over?" method. <-1>
  ;; Tests/Examples:
  ...
  (define (to-draw)
    (foldr (λ (wld scene) (place-image (wld . to-draw)
           ...
           (this . worlds)))
  ;;> You can only use the methods outlined in the "WORLD" interface
  ;;> Here you are assuming that a world has an "x" and a "y" method. <-1>

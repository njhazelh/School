;;> Graded By: Kathleen Mullins (mullinsk@ccs.neu.edu)
;;> (88/100)
...
;;> Interpretation of data for mobiletree &leaf <-2>
...
;;> Need interpretation of data for end% and possession%.
;;> the fields for possession% don't match the description of them
;;> also the next function doesn't need to exist because it will never
;;> be called in an end% and it is just a field in possession%. <-3>
...
(define-class event%
  ...
  ;;> Doesn't cover the case where x ends before this <-2>
  ;; overlap? : Event -> Boolean
  ;; determines whether this event overlaps with a given event
  (define (overlap? x)
...
;;> Again, the rest-event function isn't necessary
...
;;> overlay/underlay should produce an object that implements IShape <-2>
;; Definitions
...
;;> Score x and y posns and color should be predefined constants that can change
